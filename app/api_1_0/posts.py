...
def get_user_posts(id):
    ...
    next_ = None
    if pagination.has_next:
        next_ = url_for('api.get_user_posts', id=id, page=page+1, _external=True)
    return jsonify({
        'posts': [post.to_json() for post in posts],
        'prev': prev,
        'next': next_,
...
@api.route('/user_followed_posts/<int:id>')
def get_user_followed_posts(id):
    ...
    next_ = None
    if pagination.has_next:
        next_ = url_for('api.get_user_followed_posts', id=id, page=page+1, _external=True)
    return jsonify({
        'posts': [post.to_json() for post in posts],
        'prev': prev,
        'next': next_,
